...
% script.
    ...
    if strcmp(settings.baseline_mode, 'absolute')
        ...
        assistance_params = [];
        [cycles, times] = processRawData(markers, grfs, ...
            settings.dirs.baseline, osim_dir, settings, assistance_params);
        ...
        switch settings.data_inputs
            ...
            otherwise
                % Compute the mean value of the metric from the baseline 
                % data, & use this as the baseline going forward.
                settings.baseline = computeMeanMetric(...
                    cycles, settings.metric, true, settings.args{:});
        end

        % More handy output.
        input(['\nBaseline computation complete.\nPress return once you'... 
            ' have finished changing filenames in Nexus and D-Flow in'...
            ' preparation for HIL data collection.'], 's');
    end
    ...
    pext = optimizableVariable(...
        'pext', settings.pext_range, 'Type', 'integer');
    rise = optimizableVariable(...
        'rise', settings.rise_range, 'Type', 'integer');
    pflex = optimizableVariable(...
        'pflex', settings.pflex_range, 'Type', 'integer');
    fall = optimizableVariable(...
        'fall', settings.fall_range, 'Type', 'integer');
    ...
    while G__iteration <= settings.max_iterations - 1
        ...
        try
            ...
            beep;
            while true 
                str = input(['Press enter to retry from this '...
                    'iteration, or integer i > 1 to retry from '...
                    'iteration i.\n'], 's');
                if isempty(str)
                    G__iteration = G__iteration - 1;
                    break;
